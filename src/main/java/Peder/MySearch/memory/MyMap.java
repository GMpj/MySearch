...
public class MyMap implements Serializable {
	...
	private Entry<String, Map<Integer, Integer>>[] container;// 实际存储数据的数组对象
	 ...
	 * @return
	 */
	public boolean put(String k, Map<Integer, Integer> v) {
		// 1.计算K的hash值
		// 因为自己很难写出对不同的类型都适用的Hash算法，故调用JDK给出的hashCode()方法来计算hash值
		int hash = k.hashCode();
		// 将所有信息封装为一个Entry
		Entry<String, Map<Integer, Integer>> temp = new Entry(k, v, hash);
	...
	private void reSize(int newSize) {
		// 1.声明新数组
		Entry<String, Map<Integer, Integer>>[] newTable = new Entry[newSize];
		max = (int) (newSize * LOAD_FACTOR);
		// 2.复制已有元素,即遍历所有元素，每个元素再存一遍
		for (int j = 0; j < container.length; j++) {
			Entry<String, Map<Integer, Integer>> entry = container[j];
			// 因为每个数组元素其实为链表，所以…………
			while (null != entry) {
	 ...
	 * @return
	 */
	private boolean setEntry(Entry<String, Map<Integer, Integer>> temp,
			Entry[] table) {
		// 根据hash值找到下标
		int index = indexFor(temp.hash, table.length);
		// 根据下标找到对应元素
		Entry<String, Map<Integer, Integer>> entry = table[index];
		...
		if (null != entry) {
			...
			while (null != entry) {
				...
				if ((temp.key == entry.key || temp.key.equals(entry.key))) {
					...
					for (Integer line : temp.value.keySet()) {
						//如果该行号已经出现过则计数加1，否则计为1
						if (entry.value.containsKey(line)) {
							int num = entry.value.get(line);
							entry.value.put(line, num + 1);
						} else {
							entry.value.put(line, 1);
						}
					}
					return true;
				}
	...
	private void addEntry2Last(Entry<String, Map<Integer, Integer>> entry,
			Entry<String, Map<Integer, Integer>> temp) {
		if (size > max) {
	 ...
	 * @param table
	 */
	private void setFirstEntry(Entry<String, Map<Integer, Integer>> temp,
			int index, Entry[] table) {
	 ...
	 * @return
	 */
	public Map<Integer, Integer> get(String k) {
		Entry<String, Map<Integer, Integer>> entry = null;
		// 1.计算K的hash值
		int hash = k.hashCode();
		// 2.根据hash值找到下标
